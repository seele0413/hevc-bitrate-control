...
当前版本为 **v1.6.0**。正式入口 `multi-encode` 只生成两路结果：

- `default_h264.mp4`：只指定 `libx264`，保留 FFmpeg/libx264 原生默认参数。
- `hevc_fixed.mp4`：固定 H.265 参数方案，参数为 `CRF 36.0 · preset medium · GOP 10s · ref 6 · b-frames 8 · lookahead 90 · 无roi · 无降噪`。
...
浏览器不直接接收 HEVC。两路原生结果分别解码后，统一使用同一套低延迟 H.264 参数生成 HLS 等价预览，从而避免浏览器 HEVC MSE/HLS 不兼容导致的黑屏。网页关闭时会发送停止信标；后端连续 10 秒收不到页面心跳时自动回收该会话的全部 FFmpeg。实时页不生成固定时长样本报告。
