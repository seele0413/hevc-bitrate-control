# V2.2.0 H.264 源码直流与 H.265 实时编码工具
...
- 起播：两路共同时间轴达到约 10 秒后，从共同直播边缘之前 10 秒处同时播放；右路仅观看预览使用 `libx264 ultrafast / CRF 21`。
- 连续性：任一路远程 HLS 卡顿时两路同步暂停，实际缓冲恢复到 3 秒后从原位置继续；HLS 保留 60 个分片。
