...
负值原样返回，网页显示为“码率增加”。状态同时提供两路窗口码率、窗口字节数、H.265 编码速度、编码积压、队列深度和固定 `saving_basis`。网页按最近约 30 秒的编码积压样本计算趋势；只有积压持续增长且超过阈值时显示“积压增长”，固定积压或短时速度波动显示“实时稳定”。

## 5. HLS 与生命周期

- 源码 HLS 使用摄像头原关键帧切片；不以重新编码修正长 GOP。
- 8 秒仍未生成源码播放列表时返回启动/延迟告警。
- 两路 HLS 保留 60 个分片；H.265 预览使用 `libx264 ultrafast / CRF 21 / zerolatency / 1 秒 GOP`，只影响浏览器观看资源，不改变正式 H.265 参数或码率统计。
- HLS 播放列表使用 `no-store/no-cache` 并关闭代理缓冲；唯一命名的 TS 分片只允许浏览器私有短期缓存。
- 页面只有在两路共同时间轴达到约 10 秒后，才从共同直播边缘之前 10 秒处揭开并同时播放。
- 起播后前 2 秒固定使用 `1.0x`；之后软同步容差为 150 ms，并用 `0.98-1.02x` 将播放延迟维持在 9-11 秒。
- 任一路出现 `waiting`、`stalled` 或 HLS 恢复错误时，两路同步暂停。两路在原位置都重新拥有至少 3 秒实际缓冲后继续播放；只有原位置已经离开 60 分片保留窗口时，才双路同步重新定位到固定延迟位置。
