# H.264 原生编码与 V1.6 H.265 固定参数研究工具
...
```text
CRF 36.0 · preset medium · GOP 10s · ref 6 · b-frames 8 · lookahead 90 · 无roi · 无降噪
```
...
| H.265 固定参数方案 | `libx265` | `medium` | 36.0 | ref=6 / bframes=8 / lookahead=90 / GOP=10秒 / min=2秒 | 无 ROI、无降噪 |
...
实时后端先用一个持续 RTSP 连接解码源画面，再通过容量为 2 的共享帧队列把同一组帧送入两路原生编码器。H.264 路只指定 `libx264`；H.265 路保持 V1.6 固定参数。两路原生编码输出在任何预览转码之前独立统计最近 30 秒编码字节和节省率。

浏览器不直接播放原生 H.264/H.265 输出。后端分别解码两路原生码流，再统一使用 `libx264 ultrafast / CRF 18 / zerolatency / 1 秒 GOP` 输出 H.264 MPEG-TS HLS 等价预览；两路预览参数必须一致，预览码率不得进入核心码率比较。前端以较慢直播边界同步两路播放器，偏差超过 500 ms 时校正。页面每 2 秒发送心跳，关闭页面发送停止信标；后端 10 秒未收到心跳即停止整组进程。实时页不生成 60 秒或其他固定时长样本报告。实时预览延迟属于本机二次编码链路，不得表述为摄像头端一次编码部署延迟。页面不得回显未脱敏 RTSP 地址。
