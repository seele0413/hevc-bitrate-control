...
- 两路 HLS 保留 20 个分片；H.265 预览使用 `libx264 ultrafast / CRF 21 / zerolatency / 1 秒 GOP`，只影响浏览器观看资源，不改变正式 H.265 参数或码率统计。
- 页面只有在两路 HLS 都达到约 3 秒共同缓冲后才揭开画面并同时播放；单路播放列表就绪时仍显示准备状态。
- 起播后前 2 秒固定使用 `1.0x` 且禁止同步调速或跳转；之后软同步容差为 150 ms，超出时以 `0.98-1.02x` 调速，偏差超过 3 秒才重新定位。
