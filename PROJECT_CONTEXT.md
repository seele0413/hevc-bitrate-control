...
- `hevc_lab/__init__.py` 与 `pyproject.toml` 版本为 `2.0.0`。
...
- `hevc_lab/multi_encode.py` 的 `MULTI_ENCODE_PIPELINE_VERSION` 为 `v2.0.0`，正式流程先生成 H.264 原生编码，再生成 H.265 固定参数方案。
...
实时链路使用容量为 2 帧的共享队列；队列满时删除最旧帧，避免缓存持续积压。HLS 保留 10 个 1 秒分片，前端不等待固定 5 秒缓存，偏差超过 0.5 秒时回到两路共同直播边界附近。页面每 2 秒发送独立心跳，同时状态轮询和 HLS 分片请求也会刷新会话租约；连续 30 秒没有任何页面活动时终止整组 FFmpeg。HLS 播放器发生致命错误后会清除对应挂载状态，允许后续轮询重新连接。实时预览延迟属于本机二次编码与等价预览链路，不等同于摄像头端一次编码部署延迟。
