...
- HLS 默认保留 20 个分片，两路积累约 3 秒后才共同揭开画面并起播；单路就绪不会提前显示首帧。
- 起播后前 2 秒不执行调速或跳转；之后以 150 ms 为软同步容差，偏差超过 3 秒才重新定位。
...
- Playwright 状态机验证确认：单路 playlist 就绪时仍由覆盖层遮住画面；双路共同时间轴达到 3 秒后才对齐并揭开；起播前 2 秒固定 `1.0x`，140 ms 偏差不调速，超过容差后才进行软同步。
